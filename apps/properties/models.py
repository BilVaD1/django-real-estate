...
"""
Когда этот менеджер используется в модели, все запросы к этой модели будут фильтроваться так,
чтобы включать только те записи, для которых published_status равно True.

Предполагается, что у модели есть поле published_status, которое определяет, 
опубликован ли объект. Если published_status установлен в True, 
объект считается опубликованным и будет включен в результаты запросов.

Особенности:

Этот менеджер предоставляет удобный способ фильтрации опубликованных объектов, 
избегая необходимости вручную добавлять .filter(published_status=True) в каждый запрос.
"""
...
class Property(TimeStampedUUIDModel):
    ...
    published = PropertyPublishedManager() # кастомный менеджер, фильтрующий опубликованные объекты.
    ...
    # Метод save переопределен для автоматической генерации уникального ref_code и обработки заголовка и описания.
    ...
    # Вычисляет окончательную цену с учетом налога на основе заданного процента налога (tax).
    @property
...
class PropertyViews(TimeStampedUUIDModel):
    ...
    class Meta:
        verbose_name = "Total Views on Property" # Определяет человекочитаемое имя модели. Это имя используется, например, при отображении модели в административной панели Django.
        verbose_name_plural = "Total Property Views" # Определяет человекочитаемое имя во множественной форме для модели. Это имя также используется при отображении модели в административной панели Django.